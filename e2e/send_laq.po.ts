...
export class SendLaqPage {
    ...
    return element.all(by.tagName('#wallet option'))
      .filter((opt) => {
        return opt.getText().then((v) => {
          return this._getLaqFromOptionString(v) > 0;
        });
      });
  }
      ...
      .filter((opt) => {
        return opt.getText().then((v) => {
          return opt.getAttribute('disabled').then(status => {
            return status === null && this._getLaqFromOptionString(v) > 0;
          });
        });
      });
  }
        ...
        if (wallets.length > 0) {
          return wallets[0].click().then(() => {
            return wallets[0].getText().then((v) => {
              const walletAmount = this._getLaqFromOptionString(v) + 1;
    ...
    return dest.sendKeys('2cYALUgtwGHRcw8fyAoXaDWLJjFTxzroggx').then(() => {
      return this.getValidsWallets().then(wallets => {
        if (wallets.length > 0) {
          return wallets[0].click().then(() => {
            return wallets[0].getText().then((v) => {
              const walletAmount = this._getLaqFromOptionString(v);
              return amount.sendKeys(walletAmount.toString()).then(() => {
                return btnSend.isEnabled();
              });
            });
  ...
  _getLaqFromOptionString(option: string) {
    const value = option.slice(option.indexOf('—') + 1, option.indexOf(' LAQ'));
