...
export class WalletsPage {
  ...
  createWalletCheckValidationSeed() {
    ...
    this.fillWalletForm('Test wallet', 'test test2', 'laqpay-web-e2e-test-seed');
  ...
  createWalletCheckValidationLabel() {
    const btnCreate = element(by.buttonText('Create'));
    this.fillWalletForm('', 'laqpay-web-e2e-test-seed', 'laqpay-web-e2e-test-seed');
    return btnCreate.isEnabled();
  }

  loadWalletCheckValidationLabel() {
    const btnLoad = element(by.buttonText('Load'));
    this.fillWalletForm('', 'laqpay-web-e2e-test-seed');
    return btnLoad.isEnabled();
  }

  createExistingWallet() {
    const btnCreate = element(by.buttonText('Create'));
    this.fillWalletForm('Test create wallet', 'laqpay-web-e2e-test-seed', 'laqpay-web-e2e-test-seed');
  ...
  createWallet() {
    ...
    this.fillWalletForm(
      'Test create wallet',
      'laqpay-web-e2e-test-create-wallet-seed',
      'laqpay-web-e2e-test-create-wallet-seed'
    );
    ...
    return btnCreate.isEnabled().then(status => {
      ...
      return this.getWalletAddress('laqpay-web-e2e-test-create-wallet-seed').then((address) => {
  ...
  loadExistingWallet() {
    const btnLoad = element(by.buttonText('Load'));
    this.fillWalletForm('Test load wallet', 'laqpay-web-e2e-test-create-wallet-seed');
    return btnLoad.click().then(() => {
  ...
  loadWallet() {
    const btnLoad = element(by.buttonText('Load'));
    this.fillWalletForm('Test load wallet', 'laqpay-web-e2e-test-load-wallet-seed');
    return btnLoad.isEnabled().then(status => {
      ...
      return this.getWalletAddress('laqpay-web-e2e-test-load-wallet-seed').then((address) => {
  ...
  canUnlockWallet() {
    return element.all(by.css('.-encryption img')).first().click().then(() => {
      return this.unlockWallet('laqpay-web-e2e-test-seed').then(() => {
  ...
  unlockWallet(seedText: string = 'laqpay-web-e2e-test-create-wallet-seed') {
    const seed = element(by.css('[formcontrolname="seed"]'));
    seed.clear();
